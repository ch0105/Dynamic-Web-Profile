...
// Handle form submission to add new blog item
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO blog (image, title, description, link) VALUES ('$image', '$title', '$description', '$link')";
...
// Handle form submission to edit blog item
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit'])) {
    $id = $_POST['id'];
    $title = $conn->real_escape_string($_POST['title']);
    $description = $conn->real_escape_string($_POST['description']);
    $image = $_POST['current_image']; // Default to current image
    $link = $conn->real_escape_string($_POST['link']); // Read More link
    ...
    $sql = "UPDATE blog SET image='$image', title='$title', description='$description', link='$link' WHERE id=$id";
    if ($conn->query($sql) === TRUE) {
                ...
                <div class="gallery">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<a href="' . $row["link"] . '">Read More</a>'; // Displaying Read More link
                            echo '<br>';
                            echo '<a href="blog.php?edit=' . $row["id"] . '">Edit</a> | '; // Edit link
                            echo '<a href="blog.php?delete=' . $row["id"] . '">Delete</a>'; // Delete link
                            echo '</div></div>';
                        }
                    } else {
                        echo "0 results";
